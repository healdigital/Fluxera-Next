...
async function verifyRLSPolicies() {
  ...
  try {
    ...
    const { data: policies, error } = await supabase.rpc('pg_policies_query', {
      query: `
          SELECT 
            schemaname,
            tablename,
    ...
    const policiesWithPermissionCheck =
      policies?.filter((p: Policy) => {
    ...
    if (policiesWithChecks === totalPolicies && totalPolicies > 0) {
      results.push({
        ...
        details: policiesWithPermissionCheck.map(
          (p: Policy) => `✓ ${p.tablename}.${p.policyname} (${p.cmd})`,
        ),
      });
    } else {
      const policiesWithoutChecks =
        policies?.filter((p: Policy) => {
          const definition = p.qual || p.with_check || '';
          return !definition.includes('has_permission');
        }) || [];
      ...
      results.push({
        ...
        details: policiesWithoutChecks.map(
          (p: Policy) =>
...
async function verifyFunctionSecurity() {
  ...
  try {
    ...
    const foundFunctions =
      functions?.map((f: FunctionInfo) => f.function_name) || [];
    ...
    const functionsWithSecurity =
      functions?.filter(
        (f: FunctionInfo) =>
          f.definition.includes('SECURITY DEFINER') ||
          f.definition.includes('SECURITY INVOKER'),
      ) || [];

    // Check if DEFINER functions have search_path
    const definerFunctions =
      functions?.filter((f: FunctionInfo) => f.security_type === 'DEFINER') ||
      [];

    const definerWithSearchPath = definerFunctions.filter((f: FunctionInfo) =>
      f.definition.toLowerCase().includes('search_path'),
    );

    if (
      functionsWithSecurity.length === functions?.length &&
      ...
      results.push({
        ...
        details: functions?.map((f: FunctionInfo) => {
          const hasSearchPath = f.definition
            .toLowerCase()
            .includes('search_path');
          const needsSearchPath =
            f.security_type === 'DEFINER' && !hasSearchPath;
...
async function verifyCheckConstraints() {
  ...
  try {
    ...
    const { data: constraints, error } = await supabase.rpc(
    ...
    const requiredConstraints: ConstraintPair[] = [
    ...
    if (missingConstraints.length === 0) {
      results.push({
        ...
        details: foundConstraints.map(
          (c: ConstraintPair) => `✓ ${c.table}.${c.constraint}`,
        ),
      });
    } else {
      results.push({
        ...
        details: [
          ...foundConstraints.map(
            (c: ConstraintPair) => `✓ ${c.table}.${c.constraint}`,
          ),
          ...missingConstraints.map(
            (c) => `✗ ${c.table}.${c.constraint} - Missing`,
          ),
        ],
      });
    }
...
async function verifyPermissionsEnum() {
  ...
  try {
    // Query enum values
    const { data: enumValues, error } = await supabase.rpc('pg_enum_query', {
      query: `
    ...
    const foundPermissions =
      enumValues?.map((e: EnumValue) => e.permission) || [];
